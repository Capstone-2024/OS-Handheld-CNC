...
def aruco_tag_data(image_dir, show_detections = False):
    ...
    for file_id, image_path in image_paths.items():
        ...
        aruco_corners, aruco_ids, aruco_rejected = detector.detectMarkers(image)

        with open(os.path.join(image_dir, f"tags_{file_id}.txt"), "w") as f:
