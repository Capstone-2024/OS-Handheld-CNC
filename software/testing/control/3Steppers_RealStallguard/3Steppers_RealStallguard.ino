...
#define STALL_PIN 21 // Arduino pin that diag pin is attached to, reads when stall occurs
...
void stallInterruptZ(){ // flag set when motor stalls
...
void setup() {
  ...
  attachInterrupt(digitalPinToInterrupt(STALL_PIN), stallInterruptZ, RISING); // Interrupt on diag pin @ pin 21. Pin 21 can no longer be used for I2C
